...
def register_git_tools(mcp: FastMCP, ingest_text_fn):
    ...
    def repo_add(
        ...
        """[Git] Register a named repository connection.
    ...
    @mcp.tool()
    def repo_list() -> str:
        """[Git] List all registered repository connections. Tokens are masked."""
        try:
            with psycopg.connect(POSTGRES_DSN) as conn, conn.cursor() as cur:
                cur.execute("""
    ...
    def repo_remove(name: str) -> str:
        """[Git] Remove a registered repository connection by name."""
        try:
            with psycopg.connect(POSTGRES_DSN) as conn, conn.cursor() as cur:
                cur.execute(
